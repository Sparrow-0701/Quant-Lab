...
with st.sidebar:
    ...
    if "EXIM_KEY" in st.secrets:
        # 키가 시스템에 있으면: 사용자에게 입력창을 보여주지 않음
        st.success("🔐 API 키가 시스템에서 안전하게 로드되었습니다.")
        api = st.secrets["EXIM_KEY"]
    else:
        # 키가 없으면(로컬 테스트 등): 입력창을 보여줌
        api = st.text_input("한국수출입은행 API 키", type="password")
    # -----------------------------------------------------------
